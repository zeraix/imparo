...
fn main() {
    ...
    // What each architecture's grid-striding phases run over, as host data: the bridge derives
    // the dispatch's simdgroup count from it so no phase pays for a mostly-idle last wave.
    std::fs::write(out.join("mega_grid.h"), mega_kernels::emit_grid())
        .expect("write mega_grid.h");
...
mod mega_kernels {
    ...
    pub fn emit_grid() -> String {
        ...
        for (i, (name, p)) in progs.iter().enumerate() {
            let d = declared(p);
            o += &format!("    case {i}u:   // {name}\n");
            if d.is_empty() {
                o += "        break;   // declares no unit-striding phase\n";
                continue;
            }
            for (word, call) in d {
                o += &format!("        out[n++] = u[{word}];   // {call}\n");
            }
            o += "        break;\n";
        }
        o += "    default: break;\n    }\n    return n;\n}\n";
