...
use imparo_backend::{Backend, BufId, KnobDecl, ModelFacts, SweepKind, Workload};
...
fn time_us(b: &dyn Backend, samples: usize, reps: usize, f: &dyn Fn()) -> f64 {
    ...
    f();
    let _ = b.end();
    let gpu_one = b.last_gpu_us();
    let one = if gpu_one > 0.0 {
        gpu_one
    } else {
    ...
    let per_buf = ((FRAME_US / one.max(1.0)) as usize).clamp(1, reps);
...
pub fn measure(
    ...
    let mark = |what: &str| {
        if verbose {
            println!("  [phase] {what} at {:.2}s", t_phase.elapsed().as_secs_f64());
        }
    };
    ...
    let applicable = |d: &KnobDecl| {
        d.applies.is_none_or(|f| f(&facts)) && (allow_bits || !d.bit_affecting)
    };
    if verbose {
    ...
    let (kv_layers, deep_len) = span_ladder.map_or(
        (SPAN_LAYERS, DEEP_DECODE_POS * 2),
        |l| {
            (SPAN_LAYERS, l.iter().copied().max().unwrap_or(kv_len).max(DEEP_DECODE_POS * 2))
        },
    );
    ...
    let kv_sizes: Vec<u64> = if s.attn_layers.is_empty() {
        vec![u64::from(2 * s.n_kv * deep_hd * slots(deep_len.max(kv_len))); kv_layers]
    } else {
        s.attn_layers
            .iter()
            .map(|&(hd, window)| {
                ...
                u64::from(2 * s.n_kv * hd * slots(span.max(kv_len)))
            })
            .collect()
    };
    ...
    if span_ladder.is_some() {
        ...
        let kvw = (s.n_kv * deep_hd) as usize;
        let mut row = vec![0_u8; kvw * 2];
        for layer in 0..kv_layers as u32 {
            for j in 0..deep_len {
                let a = (j.wrapping_mul(2_654_435_761) >> 20 & 63) as f32 / 32.0 - 1.0;
                let c = (j.wrapping_mul(40_503) >> 8 & 63) as f32 / 64.0;
                for (e, pair) in row.chunks_mut(2).enumerate() {
                    let v = if e % 2 == 0 { a } else { c - a * 0.5 };
                    pair.copy_from_slice(&f16(v).to_le_bytes());
                }
                let off = u64::from(j) * (kvw as u64) * 2;
                b.write_kv_bytes(layer, false, off, &row);
                b.write_kv_bytes(layer, true, off, &row);
            }
        }
        let qn = (s.n_head * deep_hd) as usize;
        let q: Vec<f32> = (0..qn).map(|i| ((i % 61) as f32 / 61.0 - 0.5) * 0.2).collect();
        b.write(BufId::Q, 0, &q);
    }
    mark("kv staged");
    ...
    // on -- a tile that wins on a narrow row can lose on a wide one -- SLICED to the same
    // GEMM_SLICE outputs the measured workload uses. Unsliced this is ~12.5 ms in one
    // dispatch, and the warm-up runs it repeatedly until the clocks settle: that alone
    // stalled the compositor for a dozen frames a run. A dispatch cannot be preempted, so
    // the only lever on UI stutter is how much work one carries.
    let (n_in, n_out) = (s.n_embd, s.n_ff.min(GEMM_SLICE));
    let reference = || {
        time_us(b, 3, 4, &|| {
            b.matmat(WK_Q4, 0, n_in, n_out, BufId::Cur, BufId::Logits, tile);
        })
    };
    ...
    .into_iter()
    // The fallback shapes carry q4_0 explicitly: it is the only quantized kind this
    // engine has a kernel for, so it is what a shape-only fallback must mean.
    .map(|(i, o)| (0, i, o, WK_Q4))
    .collect();
    ...
    let attn_at = |n_tok: u32, pos: u32, len: u32| attn_hd(n_tok, pos, len, s.head_dim, len);
    ...
    let run_workload = |w: Workload| -> f64 {
        match w {
            Workload::DecodeMix => mix
                ...
                .sum(),
            Workload::NarrowMix(n) => time_us(b, 3, 8, &|| {
                ...
                // cannot move.
                b.matmat(s.wide_kind, 0, s.n_embd, s.n_ff, BufId::Cur, BufId::Logits, n);
                b.matmat(s.wide_kind, 0, s.n_ff, s.n_embd, BufId::Cur, BufId::Logits, n);
            }),
            ...
            Workload::AttentionDecode => time_us(b, 3, 256, &attn(1, 512)),
            ...
            Workload::AttentionDecodeDeep => {
                ...
                let saved = boundary.map(|d| (d.current)());
                ...
                let t = time_us(b, 3, 64, &attn_deep(1, deep_pos, deep_len.max(kv_len)));
                if let (Some(d), Some(v)) = (boundary, saved) {
                    (d.apply)(v);
                }
                t
            }
            Workload::AttentionPrefill => time_us(b, 3, 2, &attn(tile, 512)),
            Workload::AttentionPrefillDeep => {
                time_us(b, 3, 2, &attn_deep(tile, deep_pos, deep_len.max(kv_len)))
            }
            ...
            Workload::DecodeAttentionStep => time_us(b, 3, 4, &|| {
            ...
            Workload::AttentionPrefillReuse => {
                time_us(b, 3, 8, &attn_deep(REUSE_TAIL, deep_pos, deep_len.max(kv_len)))
            }
            ...
            //
            // SLICED to GEMM_SLICE outputs, and that is what keeps the machine usable.
            ...
            Workload::PrefillGemm => {
                let out = s.n_ff.min(GEMM_SLICE);
                time_us(b, 3, 4, &move || {
                    b.matmat(WK_Q4, 0, s.n_embd, out, BufId::Cur, BufId::Logits, tile);
                    b.matmat(WK_Q4, 0, s.n_ff, out.min(s.n_embd), BufId::Cur, BufId::Logits, tile);
                })
            }
        }
    };
    ...
    if owned.layer_work_ns == 0 {
        ...
        let mut t: Vec<f64> = (0..5).map(|_| run_workload(Workload::DecodeMix)).collect();
        ...
        let mut tp: Vec<f64> = (0..3).map(|_| run_workload(Workload::PrefillGemm)).collect();
        tp.sort_by(f64::total_cmp);
        // clamp, not min-then-max: it says the intent in one call, and GEMM_SLICE is a
        // positive constant so the panic case (max < min) cannot arise.
        let scale = f64::from(s.n_ff.max(1)) / f64::from(s.n_ff.clamp(1, GEMM_SLICE));
    ...
    let screen_probe = || {
        let t = std::time::Instant::now();
        b.begin();
        b.matmat(WK_Q4, 0, 256, 256, BufId::Cur, BufId::Logits, 8);
        let _ = b.end();
        let gpu = b.last_gpu_us();
        if gpu > 0.0 {
            gpu
    ...
    for d in reg.iter().filter(|d| {
        d.screened && applicable(d) && only.is_none_or(|k| d.name == k)
    }) {
        ...
        let saved_routing: Vec<(&KnobDecl, u32)> = reg
            ...
            .filter_map(|r| match r.sweep {
                ...
                SweepKind::Crossing { lo, .. } | SweepKind::SpanCrossing { lo, .. } => {
                    let cur = (r.current)();
                    (r.apply)(lo);
                    Some((r, cur))
                }
                SweepKind::Derived | SweepKind::Values => None,
            })
            .collect();
        ...
        for &v in &legal_here {
            ...
            let _ = screen_probe(); // untimed: pays the switch
    ...
    let sweep = |d: &KnobDecl| -> u32 {
        ...
        (d.apply)(current);
        let time_one = || run_workload(d.workload);
        ...
        cands.extend(allowed.iter().copied().filter(|v| *v != current));
        ...
        if verbose && cands.len() >= 2 {
            let mut times: Vec<f64> = (0..cands.len()).map(|i| med(&mut samples[i])).collect();
        ...
        if let Some(cw) = d.cross_check {
            let mut ranked: Vec<(usize, f64)> = (0..cands.len())
                .map(|i| (i, med(&mut samples[i])))
                .collect();
            ranked.sort_by(|a, b| a.1.total_cmp(&b.1));
            if ranked.len() >= 2 {
                let (wv, rv) = (cands[ranked[0].0], cands[ranked[1].0]);
                let on = |v: u32| {
                    (d.apply)(v);
                ...
                let (wu, ru) = (on(wv), on(rv));
                if verbose {
                    if ru < wu * (1.0 - margin) {
                        println!(
                            "  {} CROSS-CHECK FLIP on {cw:?}: {wv} wins its own regime but \
                             {rv} is {:.0}% faster here ({ru:.1} vs {wu:.1} us) -- one \
                             value does not serve both, this wants VARIANTS",
                            d.name,
                            (wu / ru - 1.0) * 100.0
                        );
                    } else {
                        println!(
                            "  {} cross-check on {cw:?}: {wv} holds ({wu:.1} vs {rv}'s \
                             {ru:.1} us)",
                            d.name
                        );
                    }
    ...
    let crossing = |d: &KnobDecl, ladder: &[u32], hi: u32, lo: u32| -> u32 {
        ...
        for _ in 0..3 {
            let (f, r) = scan();
            last = r;
            // No crossing on the ladder: the hi side won everywhere scanned.
            found.push(f.unwrap_or_else(|| *ladder.last().unwrap()));
        }
    ...
    let span_crossing = |d: &KnobDecl, ladder: &[u32], hi: u32, lo: u32| -> u32 {
        ...
        let mut samples: Vec<crate::crossing::Sample> = Vec::with_capacity(ladder.len());
        ...
        if let Some(t) = crate::crossing::threshold_from_signs(&samples) {
            if verbose {
                let band: Vec<String> = samples
                    .iter()
                    .map(|s| {
                        let rel = (s.lo_us - s.hi_us) / s.lo_us.max(1e-9) * 100.0;
                        let mark = if rel > s.noise * 100.0 {
                            "hi"
                        } else if rel < -s.noise * 100.0 {
                            "lo"
                        } else {
                            "tie"
                        };
                        format!("{}:{rel:+.2}%/{:.2}%{mark}", s.span, s.noise * 100.0)
                    })
                    .collect();
                println!(
                ...
                if verbose {
                    println!(
                        "  {}: hi = {:.0} us + {:.4}/pos, lo = {:.0} us + {:.4}/pos -> they \
                         meet at span {:.0} -> {}",
                        d.name, hi.fixed, hi.marginal, lo.fixed, lo.marginal, span, settled
                    );
                }
                settled
            }
            crate::crossing::Verdict::NearParallel { hi_marginal, lo_marginal } => {
                if verbose {
                    println!(
    ...
    let joint = |members: &[&'static KnobDecl]| -> Vec<(&'static str, u32)> {
        ...
        if !vals.is_empty() {
            ...
            if total > JOINT_MAX {
                if verbose {
                    println!(
                        "  group [{}]: {total} combinations exceeds {JOINT_MAX},                          falling back to one knob at a time",
                        vals.iter().map(|d| d.name).collect::<Vec<_>>().join(" ")
                    );
                }
                for d in &vals {
                    out.push((d.name, sweep(d)));
                }
            } else {
                ...
                if let Some(cw) = vals.iter().find_map(|d| d.cross_check) {
                    let mut ranked: Vec<(usize, f64)> = (0..combos.len())
                        .map(|i| (i, med(&mut samples[i])))
                        .collect();
                    ranked.sort_by(|a, b| a.1.total_cmp(&b.1));
                    if ranked.len() >= 2 {
                        let (wi, ri) = (ranked[0].0, ranked[1].0);
                        let on = |c: &Vec<u32>| {
                            apply(c);
                            let _ = run_workload(cw);
                            let mut v: Vec<f64> =
                        ...
                        let (wu, ru) = (on(&combos[wi]), on(&combos[ri]));
                        let flipped = ru < wu * (1.0 - margin);
                        if verbose {
                            let name = |c: &Vec<u32>| {
                                vals.iter()
                                    .zip(c)
                            ...
                            println!(
                                "  cross-check on {cw:?}: winner [{}] {wu:.1} us, \
                                 runner-up [{}] {ru:.1} us{}",
                                name(&combos[wi]),
                                name(&combos[ri]),
                                if flipped {
        ...
        for d in &rest {
            let v = match d.sweep {
                ...
                SweepKind::Derived => {
                    d.derive.map_or_else(|| (d.current)(), |f| f(&facts, profile))
                }
    ...
    mark("screen done");
    let wanted = |d: &KnobDecl| -> bool {
        applicable(d)
            && only.is_none_or(|k| {
                d.name == k
                    || d.tuple.is_some_and(|g| {
                        reg.iter().any(|o| o.name == k && o.tuple == Some(g))
                    })
        })
    };
    ...
    for d in reg.iter().filter(|d| wanted(d)) {
        ...
        let v = match d.sweep {
            ...
            SweepKind::Derived => {
                let raw = d.derive.map_or_else(|| (d.current)(), |f| f(&facts, profile));
                ...
                let v = crate::crossing::settle(f64::from(raw), (d.current)(), 1.5);
                if verbose {
                    let live = (d.current)();
                    println!(
                        "  {} = {v} (derived{}){}",
                        d.name,
                        if raw == v { String::new() } else { format!(" {raw}, held") },
                        if live == v {
                            String::new()
                        } else {
    ...
    if drift > 0.20 {
        return Err(format!(
            "the machine moved {:.0}% during stage 1 ({ref_before:.0} us -> \
             {ref_after:.0} us); these measurements are not comparable to each other. \
             Close other GPU work and re-run.",
            drift * 100.0
        ));
    }
